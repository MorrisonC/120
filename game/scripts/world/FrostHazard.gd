...
func _physics_process(delta: float) -> void:
	if is_instance_valid(_tracked_player):
		process_frost_damage(_tracked_player, delta)

func process_frost_damage(player: CharacterBody3D, delta: float) -> bool:
	var gs = game_state
	if gs != null and gs.capabilities.get("has_warmth", false):
		return false # WarmCloak protects player
