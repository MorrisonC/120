...
if [[ -f SESSION_LOG.md ]]; then
  ...
  echo "See resources/session-log-format.md to create it."
fi
...
echo ""
echo "############################################"
echo "# Visual-check queue (post-task-visual-critic-120)"
echo "############################################"
if [[ -f skills/post-task-visual-critic-120/scripts/list_visual_check_tasks.py ]]; then
  python3 skills/post-task-visual-critic-120/scripts/list_visual_check_tasks.py
else
  echo "post-task-visual-critic-120 not installed alongside this skill yet."
fi

echo ""
echo "############################################"
echo "# Reminder: verify at the right tier before marking anything DONE"
echo "# See TEST_HARNESS_ARCHITECTURE.md. If the picked task is flagged"
echo "# Visual check: yes, run post-task-visual-critic-120 before marking"
echo "# it done. Write a SESSION_LOG.md entry before ending this session"
echo "# -- see resources/session-log-format.md."
echo "############################################"
